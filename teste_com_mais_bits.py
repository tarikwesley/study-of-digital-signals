...
def generate_pam_data(order, num_symbols):
    """Gera dados aleatórios para modulação PAM."""
    return np.random.randint(0, order, size=num_symbols)

def generate_ofdm_data(num_symbols, num_carriers):
    """Gera dados aleatórios para modulação OFDM."""
    return np.random.randint(0, 16, size=(num_symbols, num_carriers))

# ===========================
# MODULAÇÃO
# ===========================
def modulate_pam(data_bits, order):
    """Modula os dados usando PAM."""
    levels = np.arange(-(order - 1), order, 2)
    symbols = levels[data_bits]
    return symbols, levels

def modulate_ofdm(qam_bits, num_carriers):
    """Modula os dados usando OFDM."""
...
def simulate_pam(order, snr_levels, num_symbols):
    """Simula a modulação PAM para diferentes níveis de SNR."""
    ber_per_snr = []
    snr_real_values = []
...
def simulate_ofdm(snr_levels, num_symbols, num_carriers):
    """Simula a modulação OFDM para diferentes níveis de SNR."""
    ofdm_bers = []
    snr_real_values = []
...
def plot_signal_evolution(original_signal, noisy_signal, modulated_signal, demodulated_signal, title):
    """Plota a evolução do sinal em diferentes estágios."""
...
def plot_snr_comparison(snr_levels, snr_real_values, title):
    """Plota o SNR desejado vs. o SNR real."""
...
# ===========================
# Exemplo de visualização do sinal PAM
data_bits = generate_pam_data(4, NUM_SYMBOLS)
pam_symbols, pam_levels = modulate_pam(data_bits, 4)
noisy_signal = add_awgn_noise(pam_symbols, 10)  # SNR fixo para visualização
decoded_bits = pam_demodulate(noisy_signal, pam_levels)
plot_signal_evolution(pam_symbols, noisy_signal, pam_symbols, decoded_bits, '4-PAM')

# Exemplo de visualização do sinal OFDM
qam_bits = generate_ofdm_data(NUM_SYMBOLS, NUM_CARRIERS)
_, ofdm_signal = modulate_ofdm(qam_bits, NUM_CARRIERS)
noisy_signal = add_awgn_noise(ofdm_signal, 10)  # SNR fixo para visualização
decoded_bits = ofdm_demodulate(noisy_signal, NUM_SYMBOLS, NUM_CARRIERS)
